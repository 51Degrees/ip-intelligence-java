...
import fiftyone.ipintelligence.cloud.data.MultiIPIntelligenceDataCloud;
import fiftyone.ipintelligence.cloud.flowelements.HardwareProfileCloudEngineBuilder;
import fiftyone.ipintelligence.shared.Constants;
...
import static fiftyone.ipintelligence.examples.shared.KeyHelper.getOrSetSuperResourceKey;
...
public class NativeModelCloud {
    ...
    static void analyseNativeModel(String nativemodel, Pipeline pipeline, PrintWriter output) throws Exception {
        ...
        try (FlowData data = pipeline.createFlowData()) {
            ...
            MultiIPIntelligenceDataCloud result = data.get(MultiIPIntelligenceDataCloud.class);
            // The 'MultiIPIntelligenceDataCloud' object contains one or more instances
            // implementing 'IPIntelligenceData' which is the same interface used for standard device
            ...
            for (IPIntelligenceData device : result.getProfiles()) {
 ...
 *
 * This example is available in full on [GitHub](https://github.com/51Degrees/ip-intelligence-java-examples/blob/master/console/src/main/java/fiftyone/devicedetection/examples/console/NativeModelCloud.java).
 */